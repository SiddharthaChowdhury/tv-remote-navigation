import type {ENavigationDirection, IFocusProviderContext} from './types';
...
class FocusListener {
    ...
    public navigate = (direction: ENavigationDirection, registeredName: string) => {
        const focusListenerName = this.getLastListenerName() || '';
        if (focusListenerName !== registeredName || this.listeners[focusListenerName] === undefined)
            return;
    ...
    public onKeyUp = (registeredName?: string) => {
        const targetListenerName = registeredName || this.getLastListenerName() || '';
        const context = this.listeners[targetListenerName];

        if (context) {
            context['triggerItemRelease']();
        }
    };

    public onKeyDown = (registeredName?: string) => {
        const targetListenerName = registeredName || this.getLastListenerName() || '';
        const context = this.listeners[targetListenerName];

        if (context) {
            context['triggerItemPress']();
        }
    };

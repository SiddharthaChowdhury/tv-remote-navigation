...
export const useFocusProvider = (contextName?: string, layer: number = 0) => {
  const focusRef = useRef<TFocusRef>({ layer });
  const { current: navMapObj } = useRef<NavigationMap>(
    new NavigationMap(contextName)
  );
